...
@DisplayName("Тестирование плейсхолдеров оплаты в рассрочку")
public class TestFormsPlaceholdersInstallment {

    private WebDriver driver;
    private InstallmentPaymentPage paymentPage;

    public static class InstallmentPaymentPage {
        ...
        private final By TAB_INSTALLMENT = By.xpath("//ul[@class='select__list']/li[3]//p");
        private final By ACTIVE_PAYMENT_FORM = By.id("pay-instalment");
        private final By INPUT_ACCOUNT_NUMBER = By.id("score-instalment");
        private final By INPUT_AMOUNT = By.id("instalment-sum");
        private final By INPUT_EMAIL = By.id("instalment-email");

        public InstallmentPaymentPage(WebDriver driver) {
            this.driver = driver;
        ...
        @Step("Переключиться на вкладку 'Оплата в рассрочку'")
        public void selectInstallmentTab() {
            ...
            wait.until(ExpectedConditions.elementToBeClickable(TAB_INSTALLMENT)).click();
    ...
    public void setup() {
        ...
        paymentPage = new InstallmentPaymentPage(driver);
        paymentPage.open();
    }

    @Test
    @DisplayName("Проверка плейсхолдеров в форме оплаты в рассрочку")
    @Description("Тест проверяет корректность текста подсказок (placeholders) в полях ввода номера счета, суммы и e-mail для формы оплаты в рассрочку.")
    public void testPlaceholdersForInstallmentServices() {
        paymentPage.selectInstallmentTab();

        assertEquals("Номер счета на 44", paymentPage.getAccountNumberPlaceholder());
